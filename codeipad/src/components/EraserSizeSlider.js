...
const EraserSizeSlider = React.memo(function EraserSizeSlider({
  value,
  min = 4,
  max = 32,
  onChange
  ...
  const handleKeyDown = useCallback((event) => {
    ...
    const nextValue = clamp(value + delta * step, min, max);
    onChange(nextValue);
  ...
  return (
    <div className="eraser-slider" aria-label="Eraser size slider">
      <div
        className="eraser-slider-track"
        ref={trackRef}
        role="slider"
        tabIndex={0}
        aria-valuemin={min}
      ...
      >
        <div
          className={`eraser-slider-thumb ${isDragging ? 'dragging' : ''}`}
          style={{ top: thumbTop }}
          onMouseDown={handleThumbMouseDown}
        />
      </div>
      <div className="eraser-slider-value">{Math.round(value)}px</div>
    </div>
  );
});
